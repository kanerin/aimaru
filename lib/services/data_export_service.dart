...
// やりたいことリスト・ふたりの質問への回答・買い物リスト・ほしいものリスト）をJSONとして書き出す。
...
class DataExportService {
  ...
  Future<String> exportAsJson(String coupleId) async {
    ...
    final shoppingItems =
        results[4].docs.map(ShoppingItem.fromDoc).map(_shoppingItemToJson).toList();
    // 予約状態（wishlistReservations）はサプライズを壊さないためあえて
    // 対象外にする。追加した本人がエクスポートすると相手の予約状況を
    // 読めてしまい、firestore.rulesで隠している意味が無くなるため。
    final wishlistItems =
        results[5].docs.map(WishlistItem.fromDoc).map(_wishlistItemToJson).toList();
