...
5. 選んだ報告の`rawText`・`summary`・`classification`を読み、内容を評価する。
   - **実装すべきか判断する。** 次のいずれかに該当する場合は実装せず、`--category`と
     ...
     rejected --category <category> --reason "..."`）。判断した理由は「実行ログの記録」に
     従ってIssue #57へも必ずコメントする（`--reason`はアプリの「送った報告」画面向けの
     簡潔な理由、Issue #57へのコメントはそれより詳しい判断過程を書いてよい）。
     ...
     - `already_done` — 既に実装済み・修正済みの内容。**「似た名前の修正コミットが過去にある」
       ...
       具体的で理解できるが、実機・実データが無いと真偽を確認できない場合（Googleカレンダー等の
       外部サービスとの実際の連携結果に依存する、特定の環境でしか再現しない、等）は、
       `unclear`ではなく`other`にした上で、`out_of_scope`と同様に`gh issue create`で
       「何を確認済みで、何が確認できなかったか」「人間に何を試してほしいか（実機での再現手順）」
       を具体的に書いたIssueを起票し、理由にそのIssue番号を書くこと。自信が無いまま
       `already_done`と断定して却下しない（実際に見た目上は妥当な過去の修正コミットが
       見つかっても、報告された具体的な症状をそれが本当にカバーしているとは限らない）。
