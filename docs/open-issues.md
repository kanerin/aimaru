...
scheduled-run  ← 定期実行の入口。安全確認して、やるかやらないかを決める
   ...
   └─ market-brief      市場調査（提案ページのみ作成）                 月次

pr-review      ← open な PR を1件レビューして直し、再提出する          5時間おき
```

**マージだけは人間が行う。** 書くのも直すのも AI になったため、独立したチェックは
PR のマージ判断しか残っていない。`pr-review` は3ラウンドで収束しなければ
エスカレーションして止まる（無限に直し続けて PR が肥大化するのを防ぐため）。
