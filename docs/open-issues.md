...
- ~~`functions`のnpm audit（moderate、`firebase-functions`→`express`→`body-parser`経由の間接依存のqs関連）~~ — 2026-09-17解消（reduce-debt枠）。GHSA-x5fp-wj9c-mxmx（array-limit bypass）・GHSA-4mjr-xmp4-gh2g（isBufferによるDoS）の対象範囲`<6.16.0`に`qs`が固定されていたため、既存のuuid overrideと同じ手法で`functions/package.json`の`overrides`に`qs: "^6.16.0"`を追加して解消した（`npm audit --omit=dev`が0件になったことを確認済み）。今回はこの実行時点で本セクション以外のP0/P1に着手可能な新規課題が無かったため、このカテゴリの棚卸しとして着手した。今後もし`overrides`無しでこの経路のバージョンが上がりqsが再び古い版に固定されたら、この対処が効かなくなる可能性がある点に留意
...
2026-09-05、市場動向調査（propose-feature）で「ふたりの質問」（QuestionsScreen）に
連続回答日数（ストリーク）表示を追加した。サービス終了したPairyの移行先として比較
されるTwinestは「付き合った日数」のカウンター表示を持つが、AIMARUの「ふたりの質問」
には日々の継続を可視化する仕組みが無く、答えるモチベーションが続きにくかった
（2026年9月時点の競合調査）。`lib/utils/question_streak.dart`の`computeQuestionStreak`
が既存の`watchRecentAnswers`（直近60件、`questions_screen_test.dart`の履歴表示と
同じデータ）から、2人とも回答した日が今日を起点に何日連続で続いているかを純粋関数で
数える。新しいFirestoreコレクション・フィールド・クエリは増やしていない。今日分が
まだ揃っていなくても昨日までの連続が保たれていれば継続中として数える（日付が変わった
直後に0へ戻ると、寝る前に回答する習慣のカップルの連続記録が毎晩リセットされて
見えてしまうため）。0日のときは何も表示せず、1日以上のときだけ「今日の質問」カードに
「🔥 n日連続」を出す。

2026-09-06、市場動向調査（propose-feature）で設定画面の「お互いを知る」セクションに
「きょうの気分」（`MoodScreen` / `MoodService`、`couples/{coupleId}/moodEntries`）を
追加した。Amora・Pairedなど関係性ウェルネス系アプリが2025〜2026年にかけて強化してきた
「気分チェックイン」が差別化要素として挙がっており（2026年9月時点の競合調査）、既存の
「ふたりの日記」（自由記述）・「ふたりの質問」（相手の回答を伏せる）よりもさらに気軽な
入口として、固定5種類の絵文字（😄最高／🙂良い／😐ふつう／😔いまいち／😢つらい）から
選ぶだけの機能にした。ドキュメントIDは`diaryEntries`と同じ`${dateKey}_$uid`（1人1日1件）
だが、選び直すだけで消したい場面が無いため`firestore.rules`では`delete`を許可していない
（`update`のみ、`uid`の書き換えは所有権の乗っ取りになるため引き続き禁止）。AI主導の
提案生成（気分に応じたデートプラン提案等）は規模が大きいため今回のスコープには含めず、
チェックイン自体とパートナー間の可視化に留めた。`DataExportService`のJSON書き出しにも
`moodEntries`を追加した。

2026-09-09、市場動向調査（propose-feature）で設定画面の「暮らし」セクションに
「ほしいものリスト」（`WishlistScreen` / `WishlistService`、`couples/{coupleId}/wishlistItems`）
を追加した。カップル・夫婦向けアプリの比較記事でプレゼント選びに使う「ほしいものリスト」が
選定基準の一つとして挙がっており（2026年9月時点の競合調査）、既存の買い物リスト（日用品・
食材）・家事分担とは別の「暮らし」機能として位置づけた。TimeTreeにはこの概念自体が無い
差別化要素。

このリストの肝は、パートナーが「用意する」を押しても、アイテムを追加した本人（欲しい側）
には一切表示されないこと（サプライズを壊さないため）。予約状態を`wishlistItems`と同じ
ドキュメントに持たせるとFirestoreのセキュリティルールはドキュメント単位でしか制御できず
本人にも見えてしまうため、`couples/{coupleId}/wishlistReservations/{itemId}`という
別コレクション（ドキュメントID = itemId）に分離した。`firestore.rules`は
`googleEventVisibility`と同じ「本人しか読み書きできない」設計を踏襲しつつ、書き込める
本人が「予約した側」（アイテムを追加した本人以外）になる点が異なる。
`WishlistService.watchMyReservedItemIds`は`where('reservedBy', isEqualTo: 自分)`で
絞り込むクエリのみを使う——これは`bugReports`の`watchMyReports`と同じ、ルールの読み取り
条件とクエリの絞り込み条件が完全に一致する「安全な」パターンで、課題3フェーズ2で見つかった
「絞り込み無しクエリはルールでは弾かれずそのまま返ってきてしまう」という既知の落とし穴を
踏まない設計にしてある。`rules_test/firestore.test.js`に、追加した本人は予約できないこと・
なりすまし予約ができないこと・予約した本人以外は読めないこと（追加した本人でも同様）を
確認するテストを追加した。

`DataExportService`のJSON書き出しには`wishlistItems`（アイテム自体）のみ追加し、
`wishlistReservations`（予約状態）は対象外にした。エクスポートは実行した本人がその場で
中身を読める形式のため、予約状態まで含めてしまうとルールで隠している意味が無くなるため。
